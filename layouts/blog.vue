    ...
    .container
      .columns
        main.column.is-6-desktop.is-offset-1-desktop
          nuxt
        aside.column.is-4
          section(v-if="sidebar" style="margin-top: 1em;")
            .card(v-if="sidebar.tagCloud" style="margin-bottom: 1em;")
              header.card-header
            ...
            .card.my-lg(v-if="sidebar.twitter" style="margin-bottom: 1em;")
              a.twitter-timeline(data-height="800" :href="'https://twitter.com/' + sidebar.twitter")
...
export default class Default extends Vue {
  ...
  tabs: any[] = JSON.parse(process.env.tabJson || '[]')
  fullUrl = process.env.baseUrl
  sidebar = JSON.parse(process.env.sidebarJson || 'null')

  q = ''

  get tags() {
    const tagList = JSON.parse(process.env['tag.listJson'] || '{}')
    ...
    return Object.keys(tagList)
      ...
      .slice(0, 30)
      .map((t) => {
        if ((process.env['tag.exclude'] || '').split(' ').includes(t)) {
          return null
        }
        return {
          name: t,
  ...
  mounted() {
    ...
    if (this.sidebar && this.sidebar.twitter && process.client) {
